...
int main() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        if(command == "mkdir") {
            ...
            if(parameter=="-rf") {
                ...
                cin >> name;
                vector<string> path = split(name, '/');
                string folder_name = path.back();//如果这个是.或者..,这种情况没有考虑
                Folder* current = find(root, name,0);
                current->father->sub_folder.erase(current->name);
                delete current;
                // if(current->sub_folder.find(folder_name) != current->sub_folder.end()) {
            ...
            string file_name = path_1.back();//如果这个是.或者..,这种情况没有考虑
            Folder* src = find(root, src_path_to_file,0);
            ...
            if(file_name != "."|file_name != "..") {
                ...
                Folder* temp = src->father;
                temp->sub_folder.erase(src->name);
                dest->sub_folder[src->name] = src;
                src->father = dest;
            }
        }
    }
    ...
    for(int i = 0; i < m; i++) {
        ...
        if(command == "cat") {
            ...
            Folder* current = find(root, name,-1);
            if(current->files.find(file_name) != current->files.end()) {
                cout << current->files[file_name]->content << endl;
            }
        }
            ...
            if(path.empty()) {
                string folder_name = ".";//如果这个是.或者..,这种情况没有考虑
                if(type.empty()&&name.empty()) {
                    search_out.push_back(path);
                }
                else if((name==folder_name|name.empty())&&(type.empty()|type=="d")) {
                    search_out.push_back(path);
                }
                ...
                string folder_name = paths.back();//如果这个是.或者..,这种情况没有考虑
                if(type.empty()&&name.empty()) {
                    search_out.push_back(path);
                }
                else if((name==folder_name|name.empty())&&(type.empty()|type=="d")) {
